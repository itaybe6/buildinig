...
export default function EmployeeAssignmentsScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-white px-4 pt-4">
      <Text className="mb-4 text-xl font-bold">קריאות שהוקצו לי</Text>
      {rows.length === 0 ? (
        <Text className="text-gray-500">אין משימות פתוחות.</Text>
      ) : (
        <View className="gap-2 pb-8">
          {rows.map((r) => (
            <Pressable
              key={r.id}
              onPress={() => router.push(`/(employee)/assignments/${r.id}`)}
